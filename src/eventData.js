const eventData= [
    {
    name:"Clan Battle (Beta)",
    banner:"https://i.ibb.co/7nq3kXF/cb.jpg",
    type:"clan",
    start:new Date("2021/3/7"),
    end:new Date("2021/3/12"),
    link:""
    },
    {
    name:"Groto 2x",
    banner:"https://i.ibb.co/chnJYJS/groto-x2.jpg",
    type:"bonus",
    start:new Date("2021/3/18"),
    end:new Date("2021/3/27"),
    link:""
    },
    {
    name:"Groto 2x",
    banner:"https://i.ibb.co/chnJYJS/groto-x2.jpg",
    type:"bonus",
    start:new Date("2021/2/25"),
    end:new Date("2021/3/7"),
    link:""
    },
    {
    name:"Groto 2x",
    banner:"https://i.ibb.co/chnJYJS/groto-x2.jpg",
    type:"bonus",
    start:new Date("2021/6/1"),
    end:new Date("2021/6/9"),
    link:""
    },
    {
    name:"Groto 3x",
    banner:"https://i.ibb.co/chnJYJS/groto-x2.jpg",
    type:"bonus",
    start:new Date("2021/4/29"),
    end:new Date("2021/5/6"),
    link:""
    },
    {
    name:"Groto 2x",
    banner:"https://i.ibb.co/chnJYJS/groto-x2.jpg",
    type:"bonus",
    start:new Date("2021/4/14"),
    end:new Date("2021/2/23"),
    link:""
    },
    {
    name:"Dungeon Mana 2x",
    banner:"https://i.ibb.co/RgkMhDs/dungeon-manax2.jpg",
    type:"bonus",
    start:new Date("2021/2/16"),
    end:new Date("2021/2/25"),
    link:""
    },
    {
    name:"Dungeon Mana 2x",
    banner:"https://i.ibb.co/RgkMhDs/dungeon-manax2.jpg",
    type:"bonus",
    start:new Date("2021/3/10"),
    end:new Date("2021/3/18"),
    link:""
    },
    {
    name:"Dungeon Mana 2x",
    banner:"https://i.ibb.co/RgkMhDs/dungeon-manax2.jpg",
    type:"bonus",
    start:new Date("2021/4/4"),
    end:new Date("2021/4/14"),
    link:""
    },
    {
    name:"Dungeon Mana 2x",
    banner:"https://i.ibb.co/RgkMhDs/dungeon-manax2.jpg",
    type:"bonus",
    start:new Date("2021/5/23"),
    end:new Date("2021/6/1"),
    link:""
    },
    {
    name:"Dungeon Mana 3x",
    banner:"https://i.ibb.co/RgkMhDs/dungeon-manax2.jpg",
    type:"bonus",
    start:new Date("2021/5/6"),
    end:new Date("2021/5/16"),
    link:""
    },
    {
    name:"Normal Drop x2",
    banner:"https://i.ibb.co/Z29vN0w/normal-questx2.jpg",
    type:"bonus",
    start:new Date("2021/2/8"),
    end:new Date("2021/2/16"),
    link:""
    },
    {
    name:"Normal Drop x2",
    banner:"https://i.ibb.co/Z29vN0w/normal-questx2.jpg",
    type:"bonus",
    start:new Date("2021/3/7"),
    end:new Date("2021/3/21"),
    link:""
    },
    {
    name:"Normal Drop x2",
    banner:"https://i.ibb.co/Z29vN0w/normal-questx2.jpg",
    type:"bonus",
    start:new Date("2021/4/23"),
    end:new Date("2021/4/29"),
    link:""
    },
    {
    name:"Normal Drop x2",
    banner:"https://i.ibb.co/Z29vN0w/normal-questx2.jpg",
    type:"bonus",
    start:new Date("2021/5/16"),
    end:new Date("2021/5/19"),
    link:""
    },
    {
    name:"Normal Drop x2",
    banner:"https://i.ibb.co/Z29vN0w/normal-questx2.jpg",
    type:"bonus",
    start:new Date("2021/6/9"),
    end:new Date("2021/6/13"),
    link:""
    },
    {
    name:"Login Bonus",
    banner:"https://i.ibb.co/rp3Qj2m/login-bonus.jpg",
    type:"bonus",
    start:new Date("2021/3/4"),
    end:new Date("2021/3/14"),
    link:""
    },
    //https://i.ibb.co/rp3Qj2m/login-bonus.jpg
    {
    name:"Rin",
    banner:"https://i.ibb.co/FHvJwjc/gacha-rin.png",
    type:"focus",
    start:new Date("2021/2/8"),
    end:new Date("2021/2/17"),
    link:"https://princess-connect.fandom.com/wiki/Rin_Morichika"
    },
    {
    name:"Jun",
    banner:"https://i.ibb.co/WyVdvGS/gacha-jun.jpg",
    type:"focus",
    start:new Date("2021/1/29"),
    end:new Date("2021/2/8"),
    link:"https://princess-connect.fandom.com/wiki/Jun_Shirogane"
    },
    {
    name:"Djeeta",
    banner:"https://i.ibb.co/b3MCFy7/gacha-djeeta.jpg",
    type:"focus",
    start:new Date("2021/1/19"),
    end:new Date("2021/1/29"),
    link:"https://princess-connect.fandom.com/wiki/Djeeta"
    },
    {
    name:"Arisa",
    banner:"https://i.ibb.co/CJvswrV/arisa.jpg",
    type:"focus",
    start:new Date("2021/2/17"),
    end:new Date("2021/3/4"),
    link:"https://princess-connect.fandom.com/wiki/Arisa"
    },
    {
    name:"Hatsune-Present",
    banner:"https://i.ibb.co/JF3dQkN/hatsune.jpg",
    type:"story",
    start:new Date("2021/2/17"),
    end:new Date("2021/3/6"),
    link:"https://princess-connect.fandom.com/wiki/Hatsune%27s_Present"
    },
    {
    name:"Little Lyrical Adventures",
    banner:"https://i.ibb.co/VSxyxGP/Little-Lyrical-Adventures.jpg",
    type:"story",
    start:new Date("2021/3/10"),
    end:new Date("2021/3/26"),
    link:"https://princess-connect.fandom.com/wiki/Little_Lyrical_Adventures"
    },
    {
    name:"Vampire Hunter with Ilya",
    banner:"https://i.ibb.co/tYb96vS/Vampire-Hunterwith-Ilya-Event.jpg",
    type:"story",
    start:new Date("2021/4/4"),
    end:new Date("2021/4/21"),
    link:"https://princess-connect.fandom.com/wiki/Vampire_Hunter_with_Ilya"
    },
    {
    name:"Dangerous Vacation",
    banner:"https://i.ibb.co/1fW0VBL/Dangerous-Vacation-Event.jpg",
    type:"story",
    start:new Date("2021/4/29"),
    end:new Date("2021/5/16"),
    link:"https://princess-connect.fandom.com/wiki/Dangerous_Vacation"
    },
    {
    name:" Deserted Island 0 Rupee Life ",
    banner:"https://i.ibb.co/3W7vdtD/Deserted-Island-Event.jpg",
    type:"story",
    start:new Date("2021/5/23"),
    end:new Date("2021/6/8"),
    link:"https://princess-connect.fandom.com/wiki/Deserted_Island_0_Rupee_Life"
    },
    {
    name:"Ayane",
    banner:"https://i.ibb.co/5kw9dYk/Ayane.jpg",
    type:"focus",
    start:new Date("2021/3/5"),
    end:new Date("2021/3/23"),
    link:"https://princess-connect.fandom.com/wiki/Ayane_Hojo"
    },
    {
    name:"Kyouka",
    banner:"https://i.ibb.co/BPd9mh8/Full-Art-Kyouka.jpg",
    type:"focus",
    start:new Date("2021/3/10"),
    end:new Date("2021/3/24"),
    link:"https://princess-connect.fandom.com/wiki/Kyouka_Hikawa"
    },
    {
    name:"Misato",
    banner:"https://i.ibb.co/0MD9cfY/Full-Art-Misato.jpg",
    type:"focus",
    start:new Date("2021/3/24"),
    end:new Date("2021/4/4"),
    link:"https://princess-connect.fandom.com/wiki/Misato_Aikawa"
    },
    {
    name:"Illya",
    banner:"https://i.ibb.co/Wcp88HD/Full-Art-Ilya.jpg",
    type:"focus",
    start:new Date("2021/4/4"),
    end:new Date("2021/4/18"),
    link:"https://princess-connect.fandom.com/wiki/Ilya_Ornstein"
    },
    {
    name:"Tsumugi",
    banner:"https://i.ibb.co/z5wwbMW/Full-Art-Tsumugi.jpg",
    type:"focus",
    start:new Date("2021/4/18"),
    end:new Date("2021/4/29"),
    link:"https://princess-connect.fandom.com/wiki/Tsumugi_Mayumiya"
    },
    {
    name:"Summer Pecorine",
    banner:"https://i.ibb.co/SwXtN2Q/Full-Art-Summer-Pecorine.jpg",
    type:"focus",
    start:new Date("2021/4/29"),
    end:new Date("2021/5/13"),
    link:"https://princess-connect.fandom.com/wiki/Pecorine/Summer"
    },
    {
    name:"Summer Suzume",
    banner:"https://i.ibb.co/xfVZ2Fw/Full-Art-Summer-Suzume.jpg",
    type:"focus",
    start:new Date("2021/5/13"),
    end:new Date("2021/5/23"),
    link:"https://princess-connect.fandom.com/wiki/Suzume/Summer"
    },
    {
    name:"Summer Kyaru",
    banner:"https://i.ibb.co/JdF71JV/Full-Art-Summer-Kyaru.jpg",
    type:"focus",
    start:new Date("2021/5/23"),
    end:new Date("2021/6/6"),
    link:"https://princess-connect.fandom.com/wiki/Kyaru/Summer"
    },
    {
    name:"Summer Tamaki",
    banner:"https://i.ibb.co/Bg4jQSY/Full-Art-Summer-Tamaki.jpg",
    type:"focus",
    start:new Date("2021/6/5"),
    end:new Date("2021/6/16"),
    link:"https://princess-connect.fandom.com/wiki/Tamaki/Summer"
    },
]

export default eventData